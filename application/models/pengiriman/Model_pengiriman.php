...
class Model_pengiriman extends CI_Model
{
   ...
   var $order = array('MsWorkplaceId' => 'asc'); // default order 
   ...
   private function _get_datatables_query()
   {
      ...
      if (isset($_POST['search']['status'])) {
         if ($_POST['search']['status'] != "")
            $this->db->where($_POST['search']['colstatus'], $_POST['search']['status']);
      } // if datatable send POST for search status

      if (isset($_POST['search']['status1'])) {
         if ($_POST['search']['status1'] != "")
            $this->db->where($_POST['search']['colstatus1'], $_POST['search']['status1']);
      } // if datatable send POST for search status

      if (isset($_POST['search']['status2'])) {
         if ($_POST['search']['status2'] != "")
            $this->db->where($_POST['search']['colstatus2'], $_POST['search']['status2']);
      } // if datatable send POST for search status
      ...
      if (isset($_POST['search']['store'])) {
         if ($_POST['search']['store'] != "")
            $this->db->where($_POST['search']['colstore'], $_POST['search']['store']);
      } // if datatable send POST for search status
      ...
      if (isset($_POST['search']['pojenis'])) {
         if ($_POST['search']['pojenis'] == "1") {
            $this->db->where("SalesRef", "");
         } else if ($_POST['search']['pojenis'] == "0") {
            $this->db->where("SalesRef !=", "");
         }
      }
      $i = 0;
