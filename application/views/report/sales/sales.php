...
<head>
   ...
   <title>Sales - <?= ($_data->MsCustomerTypeId == 1 ? $_data->MsCustomerName : $_data->MsCustomerName . ' (' . $_data->MsCustomerCompany . ')') ?></title>
   <style>
      @font-face {
...
<body>
   ...
   <div class="item">
      <table class="center" style="width: 100%;border-collapse: collapse;border-bottom: 1px solid black;margin-right:20px">
         ...
         $data = "";
         foreach ($_item as $rowsitem) { 
            $data_split_var =  explode("|",$rowsitem->SalesDetailVarian);
            $varian = "";
            foreach($data_split_var as $row1){
               $data_split_var_row =  explode(":",$row1);
               if($data_split_var_row[0] != "Vendor") $varian .= $data_split_var_row[1]." | ";
            }
            $no++;
            $data .=      '
               <tr style="font-size:13px;vertical-align: top;" >
                  ...
                  <td style="width:30px;padding-right:5px;font-size:13px;text-align: right;">' . number_format($rowsitem->SalesDetailQty, 2) . '</td>
                  <td style="width:30px;"><span style="font-size:13px;">' . $rowsitem->SatuanName . '</span></td>';
            if ($rowsitem->SalesDetailDiscTypeAll==1) { 
               $data .= '<td style="width:80px;font-size:13px;text-align: right;"><s>' . number_format($rowsitem->SalesDetailPrice) . '</s>&nbsp;' . number_format($rowsitem->SalesDetailPrice - $rowsitem->SalesDetailDisc) . '</td>';
            }else{
               $data .= '<td style="width:80px;font-size:13px;text-align: right;">' . number_format($rowsitem->SalesDetailPrice) . '</td>';
            } 
            if ($rowsitem->SalesDetailDiscTypeAll==2) { 
               $data .= '<td style="width:100px;font-size:13px;text-align: right;"><s>' . number_format(($rowsitem->SalesDetailPrice * $rowsitem->SalesDetailQty)) . '</s>&nbsp;' . number_format(($rowsitem->SalesDetailPrice * $rowsitem->SalesDetailQty) - $rowsitem->SalesDetailDiscTotal) . '</td>';
            }else{
               $data .= '<td style="width:100px;font-size:13px;text-align: right;">' . number_format($rowsitem->SalesDetailTotal) . '</td>';
            } 
            $data .= '</tr>';
         }
         if ($_optional) {
            ...
            foreach ($_optional as $rowsitems) {
               ...
               $data .= '
                  <tr style="font-size:13px;vertical-align: top;" >
                     ...
                     <td colspan="5">' . $rowsitems->SalesOptionalDesc . '</td>
   ...
   <div class="footer">
      <table style="width: 100%;">
         <tr>
            <td align="left" valign="top" style="width:12rem;font-size:13px">
               <table style="margin-top:0px">
                  <tr>
                     <td align="center" valign="top" style="width:6rem;border:1px solid black;text-decoration: underline;height:100px">Penerima/Pembeli</td>
                     <td align="center" valign="top" style="width:6rem;border:1px solid black;text-decoration: underline;height:100px">Admin</td>
                  </tr>
               </table>
            </td>
            <td align="left" valign="top" style="width:auto;font-size:11px">
               Note :
               <ol>
                  <?= $_footer ?>
               </ol>
            </td>
            <td align="left" valign="top" style="width:9rem;font-size:13px">
               <table style="width: 100%; margin-right:-2px;margin-top:0px;border-collapse: collapse;">
                  <tr>
                     <td align="right" valign="top" style="width:50%;font-size:13px;">Sub Total</td>
                     <td align="right" valign="top" style="width:50%;font-size:13px;font-weight:bold"><?= number_format($_data->SalesSubTotal) ?></td>
                  </tr>
                  <?php 
                  if ($_data->SalesDiscTotal > 0) {
                     echo '
                           <tr>
                              <td align="right" valign="top" style="width:50%;font-size:13px;">Disc</td>
                  ...
                  if ($_data->SalesDeliveryTotal > 0) {
                     echo '
                           <tr>
                              <td align="right" valign="top" style="width:50%;font-size:13px;">Delivery</td>
                              <td align="right" valign="top" style="width:50%;font-size:13px;font-weight:bold">' . number_format($_data->SalesDeliveryTotal) . '</td>
                           </tr>';
                  }
                  ?>
                  <tr>
                     <td align="right" valign="top" style="width:50%;font-size:13px;font-weight:bold">Grand Total</td>
