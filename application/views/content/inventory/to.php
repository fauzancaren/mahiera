   ...
   <section class="content-header">
      <div class="row mb-2">
         <div class="col-md-auto col-12">
            <h2>Data Kirim Barang (TO)</h1>
         </div>
         <div class="col align-self-end">
            <ol class="breadcrumb float-md-end">
               <li class="breadcrumb-item">Inventory</li>
               <li class="breadcrumb-item active">Data Kirim Barang (TO)</li>
            </ol>
         </div>
      </div>
   </section>
         ...
         <div class="card border-top-orange">
            <div class="card-header">
               <div class="row align-items-center">
                  <div class="col">
                     <span class="fw-bold"><i class="fas fa-warehouse" aria-hidden="true"></i>&nbsp;Inventory - Data Kirim Barang (TO)</span>
                  </div>
            ...
            <div class="card-body">
               <div class="row">
                  <div class="col-lg-6 col-12">
                     ...
                     <div class="row mb-1">
                        <label for="tb_store" class="col-sm-3 col-form-label">Kirim ke Toko</label>
                        <div class="col-sm-9">
                           <select class="custom-select custom-select-sm form-control form-control-sm" id="tb_store" name="tb_store" style="width:100%">
                           </select>
                        </div>
                     </div>
                     <div class="row mb-1">
   ...
   <script>
      $(document).ready(function() {
         ...
         load_dst = function(val = true) {
            ...
            data_vendor.forEach(element => {
               if ($("#tb_status").val() != element["MsWorkplaceId"]) {
                  data.push({
                     id: element["MsWorkplaceId"],
                     text: element["MsWorkplaceCode"] + " - " + element["MsWorkplaceName"]
                  });
            ...
            if (!val) {
               $('#tb_store option').remove();
               $("#tb_store").select2('destroy');
            }
            $("#tb_store").select2({
               data: data,
            })
         ...
         var table = $('#tb_data').DataTable({
            ...
            "serverSide": true,
            "ajax": {
               "url": "<?php echo site_url('function/Client_datatable_inventory/get_data_to') ?>",
               "type": "POST",
               "data": function(data) {
                  data.search['value'] = $('#tb_search').val();
                  data.search['status'] = $('#tb_status').val();
                  data.search['colstatus'] = "InvTOSrc";
                  data.search['status1'] = $('#tb_store').val();
                  data.search['colstatus1'] = "InvTODst";
                  data.search['status2'] = $('#tb_status1').val();
                  data.search['colstatus2'] = "InvTOStatus";
                  data.search['tanggalstart'] = StartDateContent.format('YYYY-MM-DD');
                  data.search['tanggalend'] = EndDateContent.format('YYYY-MM-DD');
                  data.search['coltanggal'] = "InvTODate";
               }
            },
            "order": [],
            "columnDefs": [{
         ...
         async function get_detail_item(code) {
            var res = $.ajax({
               type: "POST",
               url: "<?php echo site_url('function/Client_datatable_inventory/get_detail_to') ?>",
               data: {
                  "code": code
               },
         ...
         $("#btn-add-kunjungan").click(function(e) {
            ...
            if (!req_status) {
               $.ajax({
                  url: "<?php echo site_url('message/message_inventory/transfer_out/') ?>",
                  beforeSend: function() {
                     req_status = 1;
                  },
         ...
         edit_click = function(id) {
            if (!req_status) {
               $.ajax({
                  url: "<?php echo site_url('message/message_inventory/transfer_out_edit/') ?>" + id,
                  beforeSend: function() {
                     req_status = 1;
                  },
                  success: function(response) {
         ...
         delete_click = function(id) {
            if (!req_status) {
               ...
               swalWithBootstrapButtons.fire({
                  title: "Hapus data",
                  html: 'Anda yakin ingin menghapus data TO ini ?',
                  icon: "warning",
                  ...
                  if (result.isConfirmed) {
                     $.ajax({
                        method: "POST",
                        url: "<?= site_url("function/client_data_inventory/data_to_delete/") ?>" + id,
                        before: function() {
                           req_status_add = 1;
                        },
                        success: function(data) {
         ...
         print_click = function(po_id) {
            window.open('<?php echo site_url('export/inventory/to/') ?>' + po_id, '_blank');
         }
