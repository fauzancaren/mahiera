   ...
   <section class="content-header">
      <div class="row mb-2">
         <div class="col-md-auto col-12">
            <h2>Data Barang Rusak (IW)</h1>
         </div>
         <div class="col align-self-end">
            <ol class="breadcrumb float-md-end">
               <li class="breadcrumb-item">Inventory</li>
               <li class="breadcrumb-item active">Data Barang Rusak (IW)</li>
            </ol>
         </div>
      </div>
   </section>
         ...
         <div class="card border-top-orange">
            <div class="card-header">
               <div class="row align-items-center">
                  <div class="col">
                     <span class="fw-bold"><i class="fas fa-warehouse" aria-hidden="true"></i>&nbsp;Inventory - Data Barang Rusak (IW)</span>
                  </div>
   ...
   <script>
      $(document).ready(function() {
         ...
         var table = $('#tb_data').DataTable({
            ...
            "serverSide": true,
            "ajax": {
               "url": "<?php echo site_url('function/Client_datatable_inventory/get_data_waste') ?>",
               "type": "POST",
               "data": function(data) {
                  ...
                  data.search['coltanggal'] = "InvWasteDate";
               }
            },
            "order": [],
            "columnDefs": [{
         ...
         async function get_detail_item(code) {
            var res = $.ajax({
               type: "POST",
               url: "<?php echo site_url('function/Client_datatable_inventory/get_detail_waste') ?>",
               data: {
                  "code": code
               },
         ...
         $('#tb_data tbody').on('click', 'td.details-control', async function() {
            ...
            if (row.child.isShown()) {
               ...
               var data = await get_detail_item(row.data()[6]);
               row.child(data).show();
               tr.addClass('shown');
            }
         });
         ...
         $("#btn-add-kunjungan").click(function(e) {
            ...
            if (!req_status) {
               $.ajax({
                  url: "<?php echo site_url('message/message_inventory/item_waste/') ?>",
                  beforeSend: function() {
                     req_status = 1;
                  },
         ...
         edit_click = function(id) {
            if (!req_status) {
               $.ajax({
                  url: "<?php echo site_url('message/message_inventory/item_waste_edit/') ?>" + id,
                  beforeSend: function() {
                     req_status = 1;
                  },
                  success: function(response) {
         ...
         delete_click = function(id) {
            if (!req_status) {
               ...
               swalWithBootstrapButtons.fire({
                  title: "Hapus data",
                  html: 'Anda yakin ingin menghapus data Waste ini ?',
                  icon: "warning",
                  ...
                  if (result.isConfirmed) {
                     $.ajax({
                        method: "POST",
                        url: "<?= site_url("function/client_data_inventory/data_waste_delete/") ?>" + id,
                        before: function() {
                           req_status_add = 1;
                        },
                        success: function(data) {
         ...
         print_click = function(po_id) {
            window.open('<?php echo site_url('export/inventory/iw/') ?>' + po_id, '_blank');
         }
