   ...
   <section class="content-header">
      <div class="row mb-2">
         <div class="col-md-auto col-12">
            <h2 onclick="menuselect('absensi-Lemburan','menu-absensi')">Data Keterangan Lembur</h2>
         </div>
         <div class="col align-self-end">
            <ol class="breadcrumb float-md-end">
               <li class="breadcrumb-item">Absensi</li>
               <li class="breadcrumb-item active" onclick="menuselect('absensi-Lemburan','menu-absensi')" style="cursor:pointer">Keterangan Lembur</li>
            </ol>
         </div>
      </div>
   </section>
         ...
         <div class="card border-top-orange">
            <div class="card-header">
               <div class="row align-items-center">
                  <div class="col">
                     <span class="fw-bold"><i class="fas fa-users" aria-hidden="true"></i>&nbsp;Absensi - Keterangan Lembur</span>
                  </div>
                     ...
                     <div class="row mb-1">
                        ...
                        <div class="col-sm-9">
                           <select class="custom-select custom-select-sm form-control form-control-sm" id="tb_type" name="tb_type" style="width:100%">
                              ...
                              <?php
                              $query = $this->db->get('TblAbsenLemburType')->result();
                              foreach ($query as $key) {
                                 echo '<option value="' . $key->AbsenLemburTypeId . '" >' . $key->AbsenLemburTypeName . '</option>';
                              }
                              ?>
                           </select>
                        </div>
   ...
   <script>
      $(document).ready(function() {
         ...
         var table = $('#tb_data').DataTable({
            ...
            "serverSide": true,
            "ajax": {
               "url": "<?php echo site_url('function/Client_datatable_absen/get_data_list_lembur') ?>",
               "type": "POST",
               "data": function(data) {
                  ...
                  data.search['coltanggal'] = "AbsenLemburDate";
                  data.search['status'] = $('#tb_type').val();
                  data.search['colstatus'] = "AbsenLemburType";
               }
            },
            "columnDefs": [{
                  "orderable": false,
         ...
         $('#btn-export').click(function(e) {
            e.preventDefault();
            $.redirect('<?php echo site_url('function/client_export_absen/data_lembur_export_excel') ?>', {
         ...
         $("#btn-add").click(function(e) {
            ...
            if (!req_status) {
               $.ajax({
                  url: "<?php echo site_url('message/message_absensi/lembur_add/') ?>",
                  beforeSend: function() {
                     req_status = 1;
                  },
         ...
         edit_click = function(id) {
            if (!req_status) {
               $.ajax({
                  url: "<?php echo site_url('message/message_absensi/lembur_edit/') ?>" + id,
                  beforeSend: function() {
                     req_status = 1;
                  },
                  success: function(response) {
         ...
         delete_click = function(id) {
            if (!req_status) {
               ...
               swalWithBootstrapButtons.fire({
                  title: "Hapus data",
                  html: 'Anda yakin ingin menghapus data Lembur ini ?',
                  icon: "warning",
                  ...
                  if (result.isConfirmed) {
                     $.ajax({
                        method: "POST",
                        url: "<?= site_url("function/client_data_absen/lembur_delete/") ?>" + id,
                        before: function() {
                           req_status_add = 1;
                        },
