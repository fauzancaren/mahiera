            ...
            <div class="card border-top-orange">
                <div class="card-header">
                    <div class="row align-items-center">
                        <div class="col">
                            <span class="fw-bold"><i class="fas fa-database" aria-hidden="true"></i>&nbsp;Master Data - Pelanggan</span>
                        </div>
                        <div class="col-auto px-1">
                            ...
                            <ul class="dropdown-menu">
                                <li>
                                    <a class="dropdown-item" onclick="window.open('<?php echo site_url('function/client_export_master/data_customer_export_excel') ?>','_blank')">
                                ...
                                <li>
                                    <a class="dropdown-item" onclick="window.open('<?php echo site_url('export/datamaster/customer') ?>','_blank')">
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#tb_data').DataTable({
                ...
                "serverSide": true,
                "ajax": {
                    "url": "<?php echo site_url('function/client_datatable/get_master_customer') ?>",
                    "type": "POST",
                    "data": function(data) {
                        data.search['value'] = $('#tb_search').val();
                        data.search['status'] = $('#tb_status').val();
                        data.search['colstatus'] = "TblMsCustomerType.MsCustomerTypeId";
                        data.search['status1'] = $('#tb_status_1').val();
                        data.search['colstatus1'] = "TblMsCustomer.MsCustomerIsActive";
                    }
                },
                "order": [],
                "columnDefs": [{
            ...
            $('#btn-add-master').click(function(e) {
                ...
                if (!req_status) {
                    $.ajax({
                        url: "<?php echo site_url('message/message_master/data_customer_add/') ?>",
                        beforeSend: function() {
                            req_status = 1;
                        },
            ...
            view_click = function(id) {
                if (!req_status) {
                    $.ajax({
                        url: "<?php echo site_url('message/message_master/data_customer_view/') ?>" + id,
                        beforeSend: function() {
                            req_status = 1;
                        },
                        success: function(response) {
            ...
            edit_click = function(id) {
                if (!req_status) {
                    $.ajax({
                        url: "<?php echo site_url('message/message_master/data_customer_edit/') ?>" + id,
                        beforeSend: function() {
                            req_status = 1;
                        },
                        success: function(response) {
                            $("#dialog-box").html(response);
                            modal_action = new bootstrap.Modal(document.getElementById('modal-action-customer'));
                            modal_action.show();
            ...
            delete_click = function(id) {
                if (!req_status) {
                    $.ajax({
                        url: "<?php echo site_url('message/message_master/data_customer_delete/') ?>" + id,
                        beforeSend: function() {
                            req_status = 1;
                        },
                        success: function(response) {
            ...
            enable_click = function(id) {
                if (!req_status) {
                    $.ajax({
                        url: "<?php echo site_url('message/message_master/data_customer_enable/') ?>" + id,
                        beforeSend: function() {
                            req_status = 1;
                        },
                        success: function(response) {
