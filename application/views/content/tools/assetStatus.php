   ...
   <section class="content-header">
      <div class="row mb-2">
         <div class="col-md-auto col-12">
            <h2 onclick="menuselect('absensi-izinsakit','menu-absensi')">Data Keterangan Asset</h2>
         </div>
         <div class="col align-self-end">
            <ol class="breadcrumb float-md-end">
               <li class="breadcrumb-item">tools</li>
               <li class="breadcrumb-item active" onclick="menuselect('absensi-izinsakit','menu-absensi')" style="cursor:pointer">Status Property</li>
            </ol>
         </div>
      </div>
   </section>
         ...
         <div class="card border-top-orange">
            <div class="card-header">
               <div class="row align-items-center">
                  <div class="col">
                     <span class="fw-bold"><i class="fas fa-users" aria-hidden="true"></i>&nbsp;tools - Status Property</span>
                  </div>
                     ...
                     <div class="row mb-1">
                        ...
                        <div class="col-sm-9">
                           <select class="custom-select custom-select-sm form-control form-control-sm" id="tb_type" name="tb_type" style="width:100%">
                              ...
                              <?php
                              $query = $this->db->get('TblAssetKelolaType')->result();
                              foreach ($query as $key) {
                                 echo '<option value="' . $key->assetKelolaTypeId . '" >' . $key->assetKelolaTypeName . '</option>';
                              }
                              ?>
                           </select>
                        </div>
   ...
   <script>
      $(document).ready(function() {
         ...
         var table = $('#tb_data').DataTable({
            ...
            "serverSide": true,
            "ajax": {
               "url": "<?php echo site_url('function/client_datatable_tools/get_visit_assetStatus') ?>",
               "type": "POST",
               "data": function(data) {
                  ...
                  data.search['coltanggal'] = "assetKelolaDate";
                  data.search['status'] = $('#tb_type').val();
                  data.search['colstatus'] = "assetKelolaTypeIdRef";
               }
            },
            "columnDefs": [{
                  "orderable": false,
         ...
         $("#btn-add").click(function(e) {
            ...
            if (!req_status) {
               $.ajax({
                  url: "<?php echo site_url('message/message_tools/data_assetStatus_add/') ?>",
                  beforeSend: function() {
                     req_status = 1;
                  },
         ...
         edit_click = function(id) {
            if (!req_status) {
               $.ajax({
                  url: "<?php echo site_url('message/message_tools/data_assetStatus_edit/') ?>" + id,
                  beforeSend: function() {
                     req_status = 1;
                  },
                  success: function(response) {
         ...
         delete_click = function(id) {
            if (!req_status) {
               ...
               swalWithBootstrapButtons.fire({
                  title: "Hapus data",
                  html: 'Anda yakin ingin menghapus data status asset ini ?',
                  icon: "warning",
                  ...
                  if (result.isConfirmed) {
                     $.ajax({
                        method: "POST",
                        url: "<?= site_url("function/client_data_tools/assetStatus_delete/") ?>" + id,
                        before: function() {
                           req_status_add = 1;
                        },
