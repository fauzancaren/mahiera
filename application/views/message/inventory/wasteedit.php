   ...
   <div class="modal-dialog modal-xl">
      <div class="modal-content" name="form-action">
         <div class="modal-header bg-dark">
            <h6 class="modal-title text-white"><i class="fas fa-pencil-alt text-warning" aria-hidden="true"></i> &nbsp;Barang Rusak (IW)</h6>
            ...
            <div class="row justify-content-center">
               <div class="col-lg-6 col-11 my-1">
                  ...
                  <div class="row mb-1 align-items-center">
                     ...
                     <div class="col-sm-9">
                        <input id="TOCode" name="TOCode" type="text" class="form-control form-control-sm" value="<?= $_data->InvWasteCode ?>" readonly>
                     </div>
                  </div>
                  <div class="row mb-1 align-items-center">
               ...
               <div class="col-xl-6 col-11 my-1">
                  ...
                  <div class="row mb-1 align-items-center">
                     ...
                     <div class="col-sm-9">
                        <textarea class="form-control form-control-sm" id="TORemarks" name="TORemarks"><?= $_data->InvWasteRemarks ?></textarea>
                     </div>
                  </div>
               </div>
...
<script>
   /*  ARRAY SELECT */
   var datestart = moment("<?= $_data->InvWasteDate ?>");
   ...
   load_dst();
   $('#MsWorkplaceId').val(<?= $_data->MsWorkplaceId ?>); // Change the value or make some change to the internal state
   ...
   var counter = 1;
   var data_item = <?= JSON_ENCODE($_detail) ?>;
   data_item.forEach(data => {
      ...
      htmlItem += '       Qty<span>&nbsp;(' + data.MsItemUoM + ')</span> : <input type="text" class="input-in-table double" name="MsItemBomDetailCount" value="' + data.InvWasteDetailQty + '"/>';
      htmlItem += '   </div>';
      ...
      htmlItem += '       </div>';
      htmlItem += '   </div>';
      table_data.push([counter, htmlItem, data.MsItemId, data.MsVendorCode, data.InvWasteDetailQty]);
      counter++;
   });
   ...
   $("#btn-submit").click(function() {
      ...
      var dataheader = {
         "InvWasteCode": $("#TOCode").val(),
         "InvWasteDate": moment(datestart).format('YYYY-MM-DD'),
         "MsWorkplaceId": $("#MsWorkplaceId").val(),
         "InvWasteRemarks": $("#TORemarks").val(),
      };
      var detailitem = [];
      for (var i = 0; table_data.length > i; i++) {
         ...
         var data = {
            ...
            "InvWasteDetailQty": table_data[i][4],
            "InvWasteDetailRef": $("#TOCode").val(),
         };
      ...
      $.ajax({
         method: "POST",
         url: "<?= site_url("function/client_data_inventory/data_waste_edit/") . $_data->InvWasteId ?>",
         data: {
            "data": dataheader,
            "item": detailitem,
         },
