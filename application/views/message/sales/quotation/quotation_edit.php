   ...
   <div class="modal-dialog modal-xl">
      <div class="modal-content" name="form-action">
         <div class="modal-header bg-dark">
            <h6 class="modal-title text-white"><i class="fas fa-pencil-alt text-warning" aria-hidden="true"></i> &nbsp;Edit Penawaran (Quotation) </h6>
            ...
            <div class="row justify-content-center">
               <div class="col-lg-12 my-lg-0 col-11 my-1">
                  <div class="row mb-1 align-items-center">
                     ...
                     <div class="col-sm-10">
                        ...
                        <div class="form-check form-check-inline" data-bs-toggle="tooltip" data-bs-placement="bottom" title="CLONBLOCINDO">
               ...
               <div class="col-lg-6 col-11 my-1">  <!-- Document | pelanggan | Pengiriman -->
                  ...
                  <div class="row mb-1 align-items-center">
                     ...
                     <div class="col-sm-9">
                        <input id="QuoCode" name="QuoCode" type="text" class="form-control form-control-sm" value="<?= $quotation->QuoCode ?>" readonly>
                     </div>
                  </div>
                  <div class="row mb-1 align-items-center">
                  ...
                  <div class="row mb-1 align-items-center">
                     ...
                     <div class="col-sm-9">
                        <select class="custom-select custom-select-sm form-control form-control-sm select-modal" id="MsEmpId" name="MsEmpId" style="width:100%" disabled>
                           <?php
                           $db = $this->db->where("MsEmpIsActive=1")->get("TblMsEmployee")->result();
                           foreach ($db as $key) {
                              if ($quotation->MsEmpId == $key->MsEmpId) {
                  ...
                  <div class="row mb-1 align-items-center">
                     ...
                     <div class="col-sm-9">
                        <select class="custom-select custom-select-sm form-control form-control-sm select-modal" id="MsWorkplaceId" name="MsWorkplaceId" style="width:100%" disabled>
                           <?php
                           $db = $this->db->where("MsWorkplaceIsActive=1")->get("TblMsWorkplace")->result();
                           foreach ($db as $key) {
                              if ($quotation->MsWorkplaceId == $key->MsWorkplaceId) {
                  ...
                  <div class="row mb-1 align-items-center">
                     <label for="QuoAsign" class="col-sm-3 col-form-label">Disiapkan Oleh<sup class="error">&nbsp;*</sup></label>
                     <div class="col-sm-9">
                        <input id="QuoAsign" name="QuoAsign" type="text" class="form-control form-control-sm" value="<?= $quotation->QuoAsign ?>">
                     </div>
                  </div>
                  ...
                  <div class="row mb-1 align-items-center">
                     ...
                     <div class="col-sm-9">
                        <div class="form-check form-switch">
                           <input id="MsDeliveryIsActive" name="MsDeliveryIsActive" class="form-check-input" type="checkbox" <?= ($quotation->QuoDelStatus == 0 ? "" : "checked") ?>>
                        </div>
                     </div>
                  </div>
                  <div class="row mb-1 align-items-center">
                     ...
                     <div class="col-sm-9">
                        <div class="input-group">
                           <select class="custom-select custom-select-sm form-select form-select-sm" id="QuoDeliveryService" name="QuoDeliveryService" style="width:90%" <?= ($quotation->QuoDelStatus == 0 ? "disabled" : "") ?>>';
                              <?php
                              $db = $this->db->where("MsDeliveryIsActive=1")->get("TblMsDelivery")->result();
                              foreach ($db as $key) {
                                 if ($quotation->QuoDelService == $key->MsDeliveryId) {
                           ...
                           <button class="btn btn-success btn-sm" id="create-new-armada" type="button" <?= ($quotation->QuoDelStatus == 0 ? "disabled" : "") ?>><i class="fas fa-plus" aria-hidden="true"></i></button>
                        </div>
                     </div>
                  </div>
                  <div class="row mb-1 ">
                  ...
                  <div class="row justify-content-center mx-2 mt-2 p-2 shadow-sm card-delivery select rounded " style="position:relative">
                     <div class="col-lg-6">
                        <div class="row mb-1 align-items-center">
                           ...
                           <div class="col-sm-8">
                              <input id="QuoSubTotal" name="QuoSubTotal" type="text" class="form-control form-control-sm price-modal text-end" value="<?= $quotation->QuoSubTotal ?>" readonly>
                           </div>
                        </div>
                        <div class="row mb-1 align-items-center">
                           <label for="QuoDeliveryTotal" class="col-sm-4 col-form-label">Pengiriman</label>
                           <div class="col-sm-8">
                              <input id="QuoDeliveryTotal" name="QuoDeliveryTotal" type="text" class="form-control form-control-sm price-modal text-end" value="<?= $quotation->QuoDeliveryTotal ?>" readonly>
                           </div>
                        </div>
                     </div>
                     <div class="col-lg-6">
                        <div class="row mb-1 align-items-center">
                           ...
                           <div class="col-sm-8">
                              <input id="QuoDiscTotal" name="QuoDiscTotal" type="text" class="form-control form-control-sm price-modal text-end" value="<?= $quotation->QuoDiscTotal ?>">
                           </div>
                        </div>
                        <div class="row mb-1 align-items-center">
                           <label for="QuoGrandTotal" class="col-sm-4 col-form-label">Grand Total</label>
                           <div class="col-sm-8">
                              <input id="QuoGrandTotal" name="QuoGrandTotal" type="text" class="form-control form-control-sm price-modal text-end" value="<?= $quotation->QuoGrandTotal ?>" readonly>
                           </div>
                        </div>
                     </div>
                  </div>
...
<script>
   ...
   var del_address = [];
   var del_id = <?= $quotation->MsCustomerDeliveryId ?>;
   var datestart = moment("<?= $quotation->QuoDate ?>", 'YYYY-MM-DD');
   set_del_id = function(i) {
      del_id = i;
   }
   $("[data-bs-toggle='tooltip']").tooltip();
   $("input[name='Quoheader'][value='<?= $quotation->QuoHeader ?>']").prop('checked', true);
   ...
   get_data_delivery = function(id) {
      $.ajax({
         ...
         success: function(data) {
            del_address = data;
            load_data_delivery("panggil dari doc");
         }
      });
   }
   load_data_delivery = function(string) { 
      var htmldelivery = "";
      for (var i = 0; del_address.length > i; i++) {
         if (del_id < 0 && del_address[i]["MsCustomerDeliveryUtama"] == 1) {
            ...
            htmldelivery += '      <a class="action-label" onclick="ubah_data_delivery(' + del_address[i]["MsCustomerDeliveryId"] + ')" >Ubah</a>';
         ...
         } else if (del_id >= 0 && del_address[i]["MsCustomerDeliveryId"] == del_id) { 
            htmldelivery = '<div id="card-delivery" class="card shadow-sm card-delivery select">';
            htmldelivery += '  <input id="MsCustomerDeliveryId" value="' + del_address[i]["MsCustomerDeliveryId"] + '" style="display:none"/>';
            htmldelivery += '  <div class="p-2 ps-4">';
            htmldelivery += '      <span class="card-title fw-bold">' + del_address[i]["MsCustomerDeliveryReceive"] + '</span><br>';
   ...
   ubah_data_delivery = function(id) {
      ...
      window.ajaxRequestSingle = $.ajax({
         ...
         success: function(data) {
            ...
            set_modal_action($("#modal-action"), id, 0);
   ...
   /* ONLY SHOW IN FORM EDIT */
   $.ajax({
      method: "POST",
      dataType: "json",
      url: "<?= site_url("function/client_data_sales/get_max_customer/") ?>" + <?= $quotation->MsCustomerId ?>,
      success: function(data) {
         var selectcustomer = $("#MsCustomerId");
         var option = new Option(data.customer, data.id, true, true);
         selectcustomer.append(option).trigger("change");
   ...
   $.ajax({
      method: "POST",
      dataType: "json",
      url: "<?= site_url("function/client_data_sales/get_del_customer/") ?>" + <?= $quotation->MsCustomerDeliveryId ?>,
      success: function(data) {
         del_address = data;
         del_id = <?= $quotation->MsCustomerDeliveryId ?>;
         load_data_delivery("panggil dari startup");
      }
   });
   ...
   var number_uniq = 0;
   var data_item = []; 
   var data_item_edit = <?= JSON_ENCODE($dataitem) ?>;
   ...
   for (var i = 0; data_item_edit.length > i; i++) {
      ...
      var htmlItem = `<div class="row-table get-item action-table-single">
            <div class="d-flex item-detail" style=" font-size: 1rem;">
               <div class="flex-shrink-0">
                  <img class="lazy skeleton" data-src="${data_item_edit[i].MsProdukImage}" style="width: 5rem;background: #f1f1f1;padding: 0.25rem;height: 5rem;object-fit: contain;"> 
               </div> 
               <div class="flex-grow-1 ms-3 d-flex flex-column ps-2">
                  <span class="fw-bold">${data_item_edit[i].MsProdukCode}-${data_item_edit[i].MsProdukName}</span> 
                  ...
                  <select class="custom-select-item stocksales" id="${data_item_edit[i].MsProdukId}store" data-index="${data_item_edit[i].MsProdukId}" style="width:4rem">
                  ...
                  <div class="flex">`;
      var varian = data_item_edit[i].MsProdukVarian.split(";");
      for(var j = 0;j < varian.length;j++){
         var varheader = varian[j].split(":");
         htmlItem += ` 
                              <select class="custom-select-item variansales" id="${data_item_edit[i].MsProdukId}${varheader[0]}" data-index="${i}" data-header="${varheader[0]}" style="padding-right: 1.5rem;">`;
         var vardetail = varheader[1].split("|");
         for(var k = 0;k < vardetail.length;k++){
            var selected = "";
            for(var l = 0; l < data_item_edit[i].selected.length;l++){
               if(data_item_edit[i].selected[l] == (varheader[0]+ ":" + vardetail[k])){
                  selected = "selected";
                  break; 
               }
            }
      ...
      htmlItem += `</div> 
                        ...
                        <div class="d-flex flex-column">
                           ...
                           <div class="">
                              <div class="form-check form-check-inline pe-2" style="min-height:0px !important;margin-right:0;margin-bottom:0;">
                                 <input class="form-check-input" type="radio" name="SalesDetailDiscType2-${data_item_edit[i].MsProdukId + number_uniq}" id="${data_item_edit[i].MsProdukId + number_uniq + (numberindex +1)}" value="0" checked>
                                 <label class="form-check-label" for="${data_item_edit[i].MsProdukId + number_uniq  + (numberindex +1) }">Tidak Ada</label>
                              </div>
                              <div class="form-check form-check-inline pe-2" style="min-height:0px !important;margin-right:0;margin-bottom:0;">
                                 <input class="form-check-input" type="radio" name="SalesDetailDiscType2-${data_item_edit[i].MsProdukId + number_uniq}" id="${data_item_edit[i].MsProdukId + number_uniq + (numberindex +2)}" value="1">
                                 <label class="form-check-label" for="${data_item_edit[i].MsProdukId + number_uniq  + (numberindex +2)}">By Item</label>
                              </div>
                              <div class="form-check form-check-inline pe-2" style="min-height:0px !important;margin-right:0;margin-bottom:0;">
                                 <input class="form-check-input" type="radio" name="SalesDetailDiscType2-${data_item_edit[i].MsProdukId + number_uniq}" id="${data_item_edit[i].MsProdukId + number_uniq + (numberindex +3)}" value="2">
                                 <label class="form-check-label" for="${data_item_edit[i].MsProdukId + number_uniq  + (numberindex +3)}">By Total</label>
                              </div> 
                           </div> 
                           <div class="pt-2 align-items-center form-disc-item-${data_item_edit[i].MsProdukId + number_uniq}" style="display:none"> 
                              <div class="form-check form-check-inline pe-2" style="min-height:0px !important;margin-right:0;margin-bottom:0;">
                                 <input class="form-check-input" type="radio" name="SalesDetailDiscType-${data_item_edit[i].MsProdukId + number_uniq}"  id="${data_item_edit[i].MsProdukId + number_uniq + (numberindex +4)}" value="0" checked>
                                 <label class="form-check-label" for="${data_item_edit[i].MsProdukId + number_uniq+ (numberindex +4)}">Rp.</label>
                              </div>
                              <input type="text" class="input-in-table double me-2" name="SalesDetailDisc" style="width:100px" for="${data_item_edit[i].MsProdukId + number_uniq+ (numberindex +4)}" value=""/>
                              <div class="form-check form-check-inline pe-2" style="min-height:0px !important;margin-right:0;margin-bottom:0;">
                                 <input class="form-check-input" type="radio" name="SalesDetailDiscType-${data_item_edit[i].MsProdukId + number_uniq}"  id="${data_item_edit[i].MsProdukId + number_uniq + (numberindex +5)}" value="1">
                                 <label class="form-check-label" for="${data_item_edit[i].MsProdukId + number_uniq+ (numberindex +5)}" >%</label>
                              </div> 
                              <input type="text" class="input-in-table double " name="SalesDetailDiscPercen" style="width:40px" for="${data_item_edit[i].MsProdukId + number_uniq+ (numberindex +5)}" value=""/>
                           </div>
                           <div class="pt-2 align-items-center form-disc-total-${data_item_edit[i].MsProdukId + number_uniq}" style="display:none"> 
                              <div class="form-check form-check-inline pe-2" style="min-height:0px !important;margin-right:0;margin-bottom:0;">
                                 <input class="form-check-input" type="radio" name="SalesDetailDiscType3-${data_item_edit[i].MsProdukId + number_uniq}"  id="${data_item_edit[i].MsProdukId + number_uniq + (numberindex +4)}" value="0" checked>
                                 <label class="form-check-label" for="${data_item_edit[i].MsProdukId + number_uniq+ (numberindex + 4)}">Rp.</label>
                              </div>
                              <input type="text" class="input-in-table double me-2" name="SalesDetailDisc2" style="width:100px" for="${data_item_edit[i].MsProdukId + number_uniq+ (numberindex +4)}" value=""/>
                              <div class="form-check form-check-inline pe-2" style="min-height:0px !important;margin-right:0;margin-bottom:0;">
                                 <input class="form-check-input" type="radio" name="SalesDetailDiscType3-${data_item_edit[i].MsProdukId + number_uniq}"  id="${data_item_edit[i].MsProdukId + number_uniq + (numberindex +5)}" value="1">
                                 <label class="form-check-label" for="${data_item_edit[i].MsProdukId + number_uniq+ (numberindex + 5)}" >%</label>
                              </div> 
                              <input type="text" class="input-in-table double " name="SalesDetailDiscPercen2" style="width:40px" for="${data_item_edit[i].MsProdukId + number_uniq+ (numberindex +5)}" value=""/>
                           </div>
                        </div>     
                        <div class="d-flex flex-column">
            ...
            <a onclick="hapus_item_click(${data_item_edit[i].MsProdukId},\'${number_uniq}\')" class="text-danger pointer" title="Hapus Item"><i class="fas fa-trash-alt fa-lg pe-2"></i>Hapus</a>
         </div>`;
      ...
      arr["selected"] = data_item_edit[i].selected;
      arr["detail"] = data_item_edit[i].MsProdukDetail;
      arr["stock"] = data_item_edit[i].MsProdukStock;
      arr["price"] = data_item_edit[i].price;
      arr["qty"] = data_item_edit[i].qty;
      arr["discitemprice"] = data_item_edit[i].discitemprice;
      arr["uom"] = data_item_edit[i].uom;
      arr["pricetotal"] = data_item_edit[i].pricetotal;
      arr["typepo"] = data_item_edit[i].typepo;
      arr["cogs"] = data_item_edit[i].cogs;
      arr["discitempersen"] = data_item_edit[i].discitempersen;
      arr["discitemtype"] = data_item_edit[i].discitemtype;
      arr["disctype"] = data_item_edit[i].disctype;
      arr["disctotalprice"] =data_item_edit[i].disctotalprice;
      arr["disctotalpersen"] = data_item_edit[i].disctotalpersen;
      arr["disctotaltype"] = data_item_edit[i].disctotaltype;
      arr["category"] = data_item_edit[i].category;
   ...
   var data_optional = []; 
   var data_optional_edit = <?= JSON_ENCODE($optional) ?>;
   for (var i = 0; data_optional_edit.length > i; i++) {
      ...
      htmlItem += '   </div>';
      htmlItem += ' </div>';
      data_optional.push([htmlItem, data_optional_edit[i]['QuoOptionalDesc'], data_optional_edit[i]['QuoOptionalPrice']]);
   }
   ...
   $("#btn-submit").click(async function() {
      ...
      if (!req_status_add) {
         ...
         $.ajax({
            method: "POST",
            url: "<?= site_url("function/client_data_sales/data_quotation_edit/") . $quotation->QuoId ?>",
            data: {
               "data": dataheader,
               "item": detailitem,
            ...
            success: function(data) {
               ...
               if (data) {
                  ...
                  Swal.fire({
                     icon: 'error',
                     text: 'Perubahan data gagal disimpan',
                     showConfirmButton: false,
                     allowOutsideClick: false,
                     allowEscapeKey: false,
                     timer: 1500
