         ...
         <div class="modal-body">
            <div class="row justify-content-center">
               <div class="col-lg-6 col-11 my-1">
                  ...
                  <div class="row mb-1 align-items-center">
                     <label for="SalesCode" class="col-sm-3 col-form-label">No. Sales<sup class="error">&nbsp;*</sup></label>
                     <div class="col-sm-9">
                        <input id="SalesCode" name="SalesCode" type="text" class="form-control form-control-sm" value="<?= $_transferout->SalesCode ?>" readonly>
                     </div>
                  </div>
                  <div class="row mb-1 align-items-center">
                     <label for="TOCode" class="col-sm-3 col-form-label">No. Doc<sup class="error">&nbsp;*</sup></label>
                     <div class="col-sm-9">
                        <input id="TOCode" name="TOCode" type="text" class="form-control form-control-sm" value="<?= $_transferout->InvTOCode ?>" readonly>
                     </div>
                  </div>
                  <div class="row mb-1 align-items-center">
               ...
               <div class="col-xl-6 col-11 my-1">
                  ...
                  <div class="row mb-1 align-items-center">
                     ...
                     <div class="col-sm-9">
                        <textarea class="form-control form-control-sm" id="TORemarks" name="TORemarks"><?= $_transferout->InvTORemarks ?></textarea>
                     </div>
                  </div>
               </div>
...
<script>
   /*  ARRAY SELECT */
   var datestart = moment("<?= $_transferout->InvTODate ?>");
   ...
   load_src();
   $('#InvTOSrc').val(<?= $_transferout->InvTOSrc ?>); // Change the value or make some change to the internal state
   $('#InvTOSrc').trigger('change.select2'); // Notify only Select2 of changes 

   load_dst();
   $('#InvTODst').val(<?= $_transferout->InvTODst ?>); // Change the value or make some change to the internal state
   ...
   t.on("draw", function() {
      ...
      $('input[name="MsItemBomDetailCount"]').each(function(index) {
         $(this).val(table_data[index]["InvTODetailQty"]);
      });
   });
   ...
   var data_item_edit = <?= JSON_ENCODE($_itemref) ?>;
   var data_item = <?= JSON_ENCODE($_item) ?>;

   function get_sum_to(itemid, vendor) {
      var total = 0;
      for (var i = 0; data_item.length > i; i++) {
         if (itemid == data_item[i]['MsProdukId'] && vendor == data_item[i]['InvTODetailVarian']) {
            total += parseFloat(data_item[i]['InvTODetailQty']);
         }
      }
      return total;
   }
   for (var i = 0; data_item_edit.length > i; i++) {
      ...
      htmlItem += '       <span >' + data_item_edit[i]['SalesDetailVarian'] + '</span>';
      htmlItem += '   </div>';
      ...
      htmlItem += '       Qty<span>&nbsp;(' + data_item_edit[i]['SatuanName'] + ')</span> : <input type="text" class="input-in-table double" name="MsItemBomDetailCount" value=""/>';
      htmlItem += '   </div>';
      
      var arr = [];
      arr["html"] = htmlItem;
      arr["MsProdukId"] =  data_item_edit[i]['MsProdukId'];
      arr["InvTODetailVarian"] = data_item_edit[i]['SalesDetailVarian'];
      arr["InvTODetailQty"] = get_sum_to(data_item_edit[i]['MsProdukId'], data_item_edit[i]['SalesDetailVarian']); 
   ...
   $("#btn-submit").click(function() {
      ...
      for (var i = 0; table_data.length > i; i++) {
         ...
         if (table_data[i]["InvTODetailQty"] > 0) {
            var data = {
               "MsProdukId": table_data[i]["MsProdukId"],
               "InvTODetailVarian": table_data[i]["InvTODetailVarian"],
               "InvTODetailQty": table_data[i]["InvTODetailQty"],
      ...
      $.ajax({
         method: "POST",
         url: "<?= site_url("function/client_data_sales/data_to_edit/") . $_transferout->InvTOId ?>",
         data: {
            "data": dataheader,
            "item": detailitem,
         },
