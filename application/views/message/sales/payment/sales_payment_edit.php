   ...
   <div class="modal-dialog modal-dialog-centered">
      <form class="modal-content" name="form-action">
         <div class="modal-header bg-dark">
            <h6 class="modal-title text-white"><i class="fas fa-pencil-alt text-warning" aria-hidden="true"></i> &nbsp;Pembayaran</h6>
         ...
         <div class="modal-body">
            ...
            <div class="row mb-1 align-items-center">
               ...
               <div class="col-sm-9">
                  <select class="custom-select custom-select-sm form-control form-control-sm select-modal" id="MsMethodId" name="MsMethodId" style="width:100%">
                     ...
                     foreach ($db as $key) {
                        if ($_payment->MsMethodId == $key->MsMethodId) {
                           echo '<option value="' . $key->MsMethodId . '" selected>' . $key->MsMethodCode . ' - ' . $key->MsMethodName . '</option>';
                        } else {
            ...
            <div class="row mb-1 align-items-center">
               ...
               <div class="col-sm-9">
                  <input id="PaymentCardName" name="PaymentCardName" type="text" class="form-control form-control-sm" value="<?= $_payment->PaymentCardName ?>" placeholder="Masukan Nama ">
               </div>
            </div>
            <div class="row mb-1 align-items-center">
               <label for="PerformaType" class="col-sm-3 col-form-label">(DP)</label>
               <div class="col-sm-9">
                  <input class="form-check-input" type="checkbox" value="" id="PerformaType" name="PerformaType" <?= ($_payment->PaymentType == "1" ? "checked" : "") ?>>
               </div>
            </div>
            <div class="row mb-1 align-items-center">
               <label for="PaymentTotal" class="col-sm-3 col-form-label">Total</label>
               <div class="col-sm-9">
                  <input id="PaymentTotal" name="PaymentTotal" type="text" class="form-control form-control-sm price-modal" value="<?= $_payment->PaymentTotal ?>" placeholder="Masukan Total">
               </div>
            </div>
            <div class="row mb-1 align-items-center">
            </div>
...
<script>
   ...
   $("#Paymentfile").hide();
   $("#paymentupload").show();
   var datestart = moment("<?= $_payment->PaymentDate  ?>");
   /*  TANGGAL DOKUMENT */
   ...
   var file = null;
   var file_name = "<?= $_payment->PaymentImage ?>";
   var src = null;
   var url = "<?= base_url("asset/image/payment/") . $_id . '/' . $_payment->PaymentImage ?>";

   async function getfile() {
      unix_time = file_name.split('.').shift().toLowerCase();
      ...
      } else if (file_type == ("xlsx" || "xlx")) {
         type = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
      } else if (file_type == ("jpg" || "jpeg")) {
         type = "image/jpeg";
      } else if (file_type == "png") {
         type = "image/png";
      }
      let file = await fetch(url).then(r => r.blob()).then(blobFile => new File([blobFile], file_name, {
         type: type
      }));
      proses_file(file);
   }
   if (file_name != "") getfile();
      ...
      $("form[name='form-action']").validate({
         ...
         submitHandler: function(form) {
            if (!req_status_add) {
               ...
               $.ajax({
                  method: "POST",
                  url: "<?= site_url("function/client_data_sales/data_sales_payment_edit") ?>",
                  data: {
                     ...
                     "PaymentRef": "<?= $_payment->PaymentRef ?>",
                     "PaymentRef2": "<?= $_payment->PaymentRef2 ?>",
                     "PaymentImage": (file == null ? "" : file_name),
                     "paymentImageOld": "<?= $_payment->PaymentImage ?>",
                     "PaymentType": ($('#PerformaType').is(":checked") ? "1" : "0"), 
                  },
                  before: function() {
                     req_status_add = 1;
                  },
