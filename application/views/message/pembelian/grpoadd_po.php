            ...
            <div class="row justify-content-center">
               <div class="col-lg-6 col-11 my-1">
                     ...
                     <div class="col-sm-9">
                        <div class="input-group">
                           <select class="custom-select custom-select-sm form-select form-select-sm" id="MsVendorId" name="MsVendorId" style="width:100%" disabled>
                              <option value="<?= $_po->MsVendorId ?>"><?= $_po->MsVendorCode . ' - ' . $_po->MsVendorName ?></option>
                           </select>
                        </div>
                     </div>
                  </div>
                  <div class="row mb-1 align-items-center">
                     ...
                     <div class="col-sm-3">
                        <div class="input-group">
                           <select class="custom-select custom-select-sm form-control form-control-sm" id="MsWorkplaceId" name="MsWorkplaceId" style="width:100%" disabled>
                              <?php
                     ...
                     <label for="PODst" class="col-sm-3 col-form-label">Kirim Ke</label>
                     <div class="col-sm-3">
                        <div class="input-group">
                           <select class="custom-select custom-select-sm form-control form-control-sm" id="PODst" name="PODst" style="width:100%">
...
<script>
   ...
   $("#MsWorkplaceId").val(<?= $_po->MsWorkplaceId ?>).trigger("change");
   ...
   $.ajax({
      type: "POST",
      dataType: "json",
      url: "<?php echo site_url('function/client_data_pembelian/get_data_po/') . $_po->POId ?>",
      success: function(response) {
         ...
         if (!(response["dataheader"]["SalesRef"] == "" || response["dataheader"]["SalesRef"] == "-")) {
            ...
            html += '<span class = "card-title" >Dibuat oleh ' + response["dataheader"]["MsEmpName"] + '</span><br>';
            html += '</div></div>';
            $("#detail-customer").html(html);
         }
         table_data = [];
         ...
         for (let i = 0; i < response["datadetail"].length; i++) {
            ...
            htmlItem += '       <span>' + response["datadetail"][i]["MsItemSize"] + '</span>&nbsp;|&nbsp;' + response["datadetail"][i]["MsItemPcsM2"] + '</span>';
            htmlItem += '   </div>';
            htmlItem += '   <div class="col-lg-3 col-6">';
            htmlItem += '       Qty PO : <input type="text" class="input-in-table double" name="podetailqty" value="' + response["datadetail"][i]["PODetailQty"] + '" disabled/><span>&nbsp;(' + response["datadetail"][i]["MsItemUoM"] + ')</span>';
            htmlItem += '   </div>';
            htmlItem += '   <div class="col-lg-3 col-6">';
            htmlItem += '       Qty Receive : <input type="text" class="input-in-table double" name="grpodetailqty" value="' + response["datadetail"][i]["PODetailQty"] + '"/><span>&nbsp;(' + response["datadetail"][i]["MsItemUoM"] + ')</span>';
            htmlItem += '   </div>';
            table_data.push([counter, htmlItem, response["datadetail"][i]["MsItemId"], response["datadetail"][i]["MsVendorCode"], response["datadetail"][i]["PODetailQty"], response["datadetail"][i]["PODetailQty"]]);
         }
