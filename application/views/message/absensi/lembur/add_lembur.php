<div class="modal fade " id="modal-action" data-bs-keyboard="false" data-bs-backdrop="static">
   <div class="modal-dialog modal-dialog-centered ">
      <div class="modal-content" name="create-toko">
         <div class="modal-header bg-dark">
            <h6 class="modal-title text-white"><i class="fas fa-plus text-success" aria-hidden="true"></i> &nbsp;Tambah kelola Lembur</h5>
         ...
         <div class="modal-body">
               ...
               <div class="col-sm-9">
                  <select class="custom-select custom-select-sm form-select form-select-sm" id="MsEmployeeId" name="MsEmployeeId" style="width:100%">
                     ...
                     foreach ($_employee as $row) {
                        echo "<option value='" . $row->MsEmpId . "' " . ($this->session->userdata("MsEmpId") == $row->MsEmpId ? "selected" : "") . ">" . $row->MsEmpCode . " - " . $row->MsEmpName . "</option>";
                     }
                     ?>
                  </select>
               </div>
            ...
            <div class="row mb-1">
               <label for="AbsenLemburType" class="col-sm-3 col-form-label">Tipe</label>
               <div class="col-sm-9">
                  <select class="custom-select custom-select-sm form-select form-select-sm" id="AbsenLemburType" name="AbsenLemburType" style="width:100%">
                     ...
                     foreach ($_kelola as $row) {
                        echo "<option value='" . $row->AbsenLemburTypeId . "'  data-name='" . $row->AbsenLemburTypeName . "' >" . $row->AbsenLemburTypeName .  "</option>";
                     }
                     ?>
                  </select>
               </div>
            ...
            <div class="row mb-1">
               <label for="AbsenDate" class="col-sm-3 col-form-label">Keterangan</label>
               <div class="col-sm-9">
                  <textarea id="AbsenDesc" name="AbsenDesc" class="form-control form-control-sm" value=""></textarea>
               </div>
            </div>
         </div>
   ...
   <script>
      ...
      $("#btn-submit").click(function() {
         $.ajax({
            method: "POST",
            url: "<?= site_url("function/client_data_absen/lembur_add") ?>",
            data: {
