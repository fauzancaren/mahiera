...
; (function ($, window, document, undefined) {
  ...
  ComboTree.prototype.init = function () {
    ...
    this._elemArrowBtn = $('<div id="' + this.comboTreeId + 'ArrowBtn" class="comboTreeArrowBtn d-none" type="button"><span class="fas fa-chevron-down comboTreeArrowBtnImg"></span></div>');
  ...
  ComboTree.prototype.createFilterHTMLForMultiSelect = function () {
    return '<input id="' + this.comboTreeId + 'MultiFilter" type="text" class="multiplesFilter" placeholder="Pencarian..."/>';
  }
  ...
  ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
    ...
    if (isThereSubs)
      itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse || collapse ? '<span class="fas fas fa-chevron-circle-right"></span>' : '<span class="fas fa-chevron-circle-down"></span>') + '</span>'; // itemHtml += '<span class="comboTreeParentPlus">' + (this.options.collapse ? '+' : '&minus;') + '</span>';

    if (this.options.isMultiple)
      itemHtml += '<span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" class="comboTreeItemTitle ' + selectableClass + '">' + (!selectableLastNode && isSelectable ? '<input type="checkbox" class="form-check-input"/>' : '') + sourceItem.title + '</span>';
    else
  ...
  ComboTree.prototype.toggleSelectionTree = function (item, direction) {
    ...
    if (direction === undefined) {
      if ($(subMenu).is(':visible'))
        $(item).children('span.comboTreeParentPlus').html('<span class="fas fa-chevron-circle-right"></span>'); //$(item).children('span.comboTreeParentPlus').html("+");
      else
        $(item).children('span.comboTreeParentPlus').html('<span class="fas fa-chevron-circle-down"></span>'); //$(item).children('span.comboTreeParentPlus').html("&minus;");
  ...
  ComboTree.prototype.refreshInputVal = function () {
    ...
    if (count == 0) {
      ...
      if (this.options.isMultiple) {
        this._elemInput.val(count + " item yang dipilih");
      } else {
